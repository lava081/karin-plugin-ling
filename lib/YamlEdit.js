 ...
 * @returns 是否继续处理
 */
const updateCfg = async (e, type, isRemoval, targetType, id) => {
  try {
    ...
    if (!isRemoval) {
      if (targetType === 'Group') {
        ...
        if ((type === 'White' && !Cfg.App.WhiteList.users) || (type === 'Black' && !Cfg.App.BlackList.users)) e.reply(`当前${type === 'White' ? '白' : '黑'}名单中的用户配置未开启，黑白名单将无效，可前往config/config/App.yaml中启用`)
      }
...
const UpdateFile = async (e, 文件, 修改的值, 配置项, 修改内容) => {
  try {
